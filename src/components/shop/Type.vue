    ...
    export default {
      ...
      ,methods:{
        ...
        toupdate(data){
          ...
          this.$axios.post("http://localhost:8080/api/type/update",this.$qs.stringify(this.form)).then(res=>{
        ...
        remove(node, data) {
          ...
          if (node.childNodes.length=="") {
            this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
              ...
              this.$axios.post("http://localhost:8080/api/type/update?isDel=1&id="+data.id).then(res=>{
                this.$message({
                  type: 'success',
                  message: '删除成功!'
                });
            ...
            this.$axios.post("http://localhost:8080/api/type/add",this.$qs.stringify(this.form)).then(res=>{
